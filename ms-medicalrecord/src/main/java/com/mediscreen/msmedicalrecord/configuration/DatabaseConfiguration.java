...
public class DatabaseConfiguration implements DatabaseConfigurationInterface {
	...
	public MongoClient getConnexion() {
		...
		return new MongoClient(new MongoClientURI("mongodb://" + this.dbConnection.getUser() + ":"
				+ this.dbConnection.getPassword() + "@" + this.dbConnection.getHost() + ":"
				+ this.dbConnection.getPort() + "/" + this.dbConnection.getDatabase() + "?authSource=admin"));
	}
	...
	public MongoDatabase getDatabase(MongoClient connexion) {
		...
		if (StringUtils.isBlank(this.dbConnection.getDatabase()))
