...
public class MedicalReportController {
    ...
    public MedicalReport generateMedicalReportById(@RequestHeader("token") String token,  @RequestParam(required = true) Map<String, Object> body) {
        return this.generate(token, Integer.valueOf((String) body.get("patId")), null);
    }

    @PostMapping("/assess/familyName")
    public MedicalReport generateMedicalReportByName(@RequestHeader("token") String token,  @RequestParam(required = true) Map<String, Object> body) {
    ...
    private MedicalReport generate(String token, Integer id, String name) {
        ...
        if(medicalReport == null) throw new NotFoundException("No report available");
